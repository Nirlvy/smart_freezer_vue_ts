...
const capture = () => {
  ...
  data.img = canvas.toDataURL('image/jpg')
  data.dialog = false
}
...
const upload = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid) => {
    if (valid) {
      client.send('realtime_rec', data.img + '-' + data.name)
      ElMessage.success('发送成功')
      client.subscribe('realtime_login')
      client.onMessageArrived = (message) => {
        if (message.destinationName === 'realtime_login') {
